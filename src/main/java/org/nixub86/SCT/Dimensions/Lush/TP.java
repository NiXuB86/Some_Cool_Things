...
public class TP extends Teleporter {
	 ...
	 private final WorldServer worldServerInstance;
	            ...
	            for (int l = -2; l <= 2; ++l)
	            {
	                for (int i1 = -2; i1 <= 2; ++i1)
	                {
	                    for (int j1 = -1; j1 < 3; ++j1)
	                    {
	                        ...
	                        int i2 = k + i1 * b1 - l * b0;
	                        boolean flag = j1 < 0;
	                        this.worldServerInstance.setBlock(k1, l1, i2, flag ? BlocksSCT.TriniumOre : Blocks.air);
	                    }
	                }
	            }
		...
		public boolean placeInExistingPortal(Entity p_77184_1_, double p_77184_2_, double p_77184_4_, double p_77184_6_, float p_77184_8_)
	    {
	        ...
	        {
	            TP.PortalPosition portalposition = (TP.PortalPosition)this.destinationCoordinateCache.getValueByKey(j1);
	            d3 = 0.0D;
	        ...
	        if (d3 >= 0.0D)
	        {
	            if (flag)
	            {
	                this.destinationCoordinateCache.add(j1, new TP.PortalPosition(i, j, k, this.worldServerInstance.getTotalWorldTime()));
	            ...
	            for (i3 = -1; i3 <= 1; ++i3)
	            {
	                for (j3 = 1; j3 < 3; ++j3)
	                {
	                    for (k3 = -1; k3 < 3; ++k3)
	                    {
	                        ...
	                        j4 = k2 + (j3 - 1) * l2 - i3 * l5;
	                        flag = k3 < 0;
	                        this.worldServerInstance.setBlock(l3, i4, j4, flag ? BlocksSCT.TriniumOre : Blocks.air);
	                    }
	                }
	            }
	        }
	        ...
	        for (i3 = 0; i3 < 4; ++i3)
	        {
	            for (j3 = 0; j3 < 4; ++j3)
	            {
	                for (k3 = -1; k3 < 4; ++k3)
	                {
	                    ...
	                    this.worldServerInstance.setBlock(l3, i4, j4, (Block)(flag ? BlocksSCT.TriniumOre : BlocksSCT.BlockTP), 0, 2);
	                }
	            }
	    ...
	    public void removeStalePortalLocations(long p_85189_1_)
	    {
	        if (p_85189_1_ % 100L == 0L)
	        {
	            ...
	            while (iterator.hasNext())
	            {
	                ...
	                TP.PortalPosition portalposition = (TP.PortalPosition)this.destinationCoordinateCache.getValueByKey(olong.longValue());
